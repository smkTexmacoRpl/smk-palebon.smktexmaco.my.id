...
<section id="blog" class="py-12 bg-gray-50 dark:bg-gray-900 mt-[20vh]">
    <div class="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-4"> Kalender Akademik SMK Palebon Semarang
            </h1>
            @if(!empty($kalender->path))
            <iframe src="{{ asset('storage/'.$kalender->path) }}#toolbar=0" type="application/pdf" frameborder="0"
                width="100%" height="600px"></iframe>
            @else
            <p class="text-gray-600 dark:text-gray-300">Kalender akademik belum diunggah.</p>
            @endif

            {{-- <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-6">
                <i data-lucide="calendar" class="w-4 h-4 mt-5 ml-[-4]"></i>
                <span>{{ $jadwal->updated_at->format('d F Y') }}</span>
            </div> --}}
        </div>
